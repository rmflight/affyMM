...
```{r insertChipImage, echo=FALSE, fig.cap="chip"}
...
```{r insertProbeSet, echo=FALSE, fig.cap="probeset"}
...
tm.image <- ggplot(pm.mm.Int, aes(x=x, y=class, fill=fill)) + geom_tile() + scale_fill_gradient(low="black", high="white", limits=c(0, 256)) + theme(axis.text.x=element_blank(), axis.ticks=element_blank(), axis.title.x=element_blank(), axis.title.y=element_blank()) + coord_equal(ratio=1, xlim=xlim) + theme(legend.position = "none")
print(tm.image)
```
...
print(xtable(cntTable), type = 'html', html.table.attributes = '', include.rownames=FALSE)
```
...
print(xtable(typeCounts), type = 'html', html.table.attributes = 'style="font-size:80%; text-align:right"', include.rownames=FALSE)
```
...
g <- ggplot(plotData, aes(x=x, y=y, colour=colour, linetype=linetype)) + geom_line() + facet_wrap(~organism, ncol=4)
print(g)
nPlot <- length(plots)
