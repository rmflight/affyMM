...
# 	cntPM <- hist(trimMult$cnt[trimMult$class == "pm"], breaks=10, plot=F)
		...
		if (sum(exonSingleMM) != 0){
			...
			for (iPM in 1:nPMs){
				...
				for (iNeg in 1:nNeg){
					...
					bl.Plot2 <- ggplot() + geom_density(data=probeSetInt, aes(x=intensity, colour=probeClass)) + geom_point(data=investigateInt, aes(x=intensity, y=y, colour=probeClass), size=3) + ggtitle(plot1Title_use)
					...
					bl.image <- ggplot(pm.mm.data, aes(x=xloc, y=probeClass, fill=fill)) + geom_tile() + scale_fill_gradient(low="black", high="white", limits=c(0, 256)) + coord_equal(ratio=1, xlim=xlim) + theme(axis.text.x=element_blank(), axis.ticks=element_blank(), axis.title.x=element_blank(), axis.title.y=element_blank()) + ggtitle(plot1Title_use)
					...
					if (length(otherProbes) > 1){
						...
						otherPlot <- ggplot(otherInt, aes(x=intensity)) + geom_point(data=mmInt, aes(x=intensity, y=y), size=4, colour="red") + ggtitle(otherPlotTitle_use)
						if (nrow(otherInt) >= 5){
	...
	returnDat <- list(pmDifDat=pmDifDat, allPlot=outPlotAllExp, negDat=outDat)
		
	} else {
		returnDat <- list(pmDifDat=NA, allPlot=NA, negDat=outDat)
	}
...
yeastPlot <- ggplot(allInt, aes(x=probeInt, colour=probeType)) + geom_density() + labs(title=inData)
```
