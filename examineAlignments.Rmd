...
# 	cntPM <- hist(trimMult$cnt[trimMult$class == "pm"], breaks=10, plot=F)
	...
	# and examine some properties
	{
		...
		# this looks at the mm and pm distribution without the interfering probes in the density calculation
		...
		rm(probeSetDen, investigateInt)
	}
	
	# so, what are the other probes that belong with the mm probe on an exon?
	...
	exonIntervals[exonHit]
	useProbes <- elementMetadata(probeAlign[otherHit])$probeNames
	
	pIntensity[useProbes, ]
