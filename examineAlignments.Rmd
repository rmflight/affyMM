...
cntDat <- matrix(0, nrow=nFile, ncol=6)
colnames(cntDat) <- c("nPM", "nMM", "dupPM", "dupMM", "singPM", "singMM")

for (iFile in 1:nFile){
	tmpSeq <- readGappedAlignments(file.path("seqData", bamFile[iFile]), use.names=T)
	nAlign <- length(names(tmpSeq))
	uniqProbes <- unique(names(tmpSeq))
	probeClass <- sapply(strsplit(uniqProbes, ".", fixed=T), function(y){y[1]})
	cntDat[iFile, "nPM"] <- sum(probeClass == "pm") # number of PM probes that align
	cntDat[iFile, "nMM"] <- sum(probeClass == "mm") # number of MM probes that align
	
	# remove duplicates first
	allProbes <- names(tmpSeq)
	dupProbes <- unique(allProbes[duplicated(allProbes)])
	
	dupFile <- paste(strsplit(bamFile[iFile], ".", fixed=T)[[1]][1], "_multMatch.txt", sep="", collapse="")
...
```{r examinData}
useData <- c("dm3", "dr6", "xtropicalis", "rn4", "hg19", "mm9", "mm10", "ce6", "yeast")
celLoc <- c(hg19="hs", mm9="mm", mm10="mm", rn4="rn", dr6="dr", dm3="dm", ce6="ce", xtropicalis="xt", yeast="yeast")

allData <- lapply(useData[1:2], function(inData){
...
# 	cntPM <- hist(trimMult$cnt[trimMult$class == "pm"], breaks=10, plot=F)
	...
	# and examine some properties
	{
		...
		exonOverlaps <- as.matrix(findOverlaps(probeAlign, exonIntervals, type="any", minoverlap=23))
		
		exonDat <- allCnts[!(is.na(allCnts$pm.E)),]
		...
		if (sum(exonSingleMM) != 0){
			...
			useDifs <- exonSingleDifs[nNeg > 0,]
			
			usePMs <- rownames(useDifs)
			useDat <- exonDatSingleMM[(exonDatSingleMM$pmProbe %in% usePMs),]
			nPMs <- nrow(useDifs)
			
			pmDifDat <- vector("list", nPMs)
			
			for (iPM in 1:nPMs){
				probeSetP <- strsplit(usePMs[iPM], ".", fixed=T)
				...
				plot1Title <- paste("ProbeSetPlot", plot1Title, collapse="::", sep="::")
				
				otherPlotTitle <- paste("ExonMatchedPlot", investigateProbes[2], collapse="::", sep="::")
				
				# and the probes for the exon that go with the mm probe
				mmLoc <- which(elementMetadata(probeAlign)$probeNames %in% investigateProbes[2])
				exonHit <- exonOverlaps[exonOverlaps[,1] == mmLoc, 2]
				otherHit <- exonOverlaps[exonOverlaps[,2] == exonHit, 1]
				otherProbes <- elementMetadata(probeAlign)$probeNames[otherHit]
				...
				for (iNeg in 1:nNeg){
					...
					plot1Title <- paste(plot1Title, celNames[iNeg], collapse="::", sep="::")
					otherPlotTitle <- paste(otherPlotTitle, celNames[iNeg], collapse="::", sep="::")
					...
					bl.Plot1 <- ggplot() + geom_histogram(data=probeSetInt, aes(x=intensity, fill=probeClass), alpha=.5, position="identity") + geom_point(data=investigateInt, aes(x=intensity, y=y, colour=probeClass), size=3) + ggtitle(plot1Title)
					bl.Plot2 <- ggplot() + geom_density(data=probeSetInt, aes(x=intensity, colour=probeClass)) + geom_point(data=investigateInt, aes(x=intensity, y=y, colour=probeClass), size=3) + ggtitle(plot1Title)
					
					
					if (length(otherProbes) != 1){
					
						otherInt <- pIntensity[otherProbes, isNeg[iNeg]]
						otherInt <- data.frame(intensity=otherInt, probeID=names(otherInt), y=0)
						
						mmInt <- otherInt[investigateProbes[2],]
						
					
						otherPlot <- ggplot(otherInt, aes(x=intensity)) + geom_point(data=mmInt, aes(x=intensity, y=y), size=4, colour="red") + ggtitle(otherPlotTitle)
						if (nrow(otherInt) >= 5){
							otherPlot <- otherPlot + geom_density()
						} else {
							otherPlot <- otherPlot + geom_histogram()
						}
					} else { 
						otherPlot <- NA
						otherInt <- NA
					...
					plotDat[[iNeg]] <- list(probeSetInt=probeSetInt, investigateInt=investigateInt, otherInt=otherInt,
																		histPlot=bl.Plot1, denPlot=bl.Plot2, exonPlot=otherPlot)	
			...
			save(.sessionInfo, .timedate, pmDifDat, file=outFile)
	...
	return(list(pmDifDat=pmDifDat, allPlot=outPlotAllExp, negDat=outDat))
	
	} else {
		return(list(pmDifDat=NA, allPlot=NA, negDat=outDat))
	}
