...
tableCount <- incCount(tableCount, "t.probeClass")
```
...
typeCounts <- matrix(0, nrow=nOrg, ncol=5)
...
for (iOrg in 1:nOrg){
	...
	typeCounts[iOrg, "_x_at"] <- length(x_at)
	typeCounts[iOrg, "_s_at"] <- length(s_at)
	typeCounts[iOrg, "_a_at"] <- length(a_at)
	typeCounts[iOrg, "_at"] <- length(o_at)
	typeCounts[iOrg, "other"] <- otherProbes
}
...
`_`s`_`at classes.  However, as `r I(pasteLabel("Table", tableCount, "t.probeClass"))` shows, the larger genomes actually contain the greatest percentage in the
...
intensities within the corresponding exonic sequences (`r I(pasteLabel("Figure", figCount, "f.allPlots"))`).  As these plots indicate, MM probes mapping within
